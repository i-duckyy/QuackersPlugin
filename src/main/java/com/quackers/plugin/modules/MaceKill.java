...
public class MaceKill extends Module {
    ...
    private final Setting<Boolean> maxPower = settingsGroup.add(new BoolSetting.Builder()
        .name("Force Max Power (Paper/Spigot ONLY)")
        .description("Simulates a fall from the highest air gap within 170 blocks")
        .defaultValue(false)
        .build());

    private final Setting<Integer> fallHeight = settingsGroup.add(new IntSetting.Builder()
    ...
    private final Setting<Boolean> packetDisable = settingsGroup.add(new BoolSetting.Builder()
        .name("Disable When Blocked")
        .description("Does not send movement packets if the attack was blocked. (prevents death)")
        .defaultValue(true)
        .build());

    private final Setting<Integer> launchCount = settingsGroup.add(new IntSetting.Builder()
        .name("Launch Count")
        .description("Number of launch packets to send per hit")
        .defaultValue(1)
        .min(1)
        .max(5)
        .build());
        ...
        if (mc.player != null
            ...
            try {
                if (((IPlayerInteractEntityC2SPacket) packet).meteor$getEntity() instanceof LivingEntity targetEntity) {
                    ...
                    double deltaY = mc.player.getY() - targetEntity.getY();
                    int blocks;

                    if (deltaY > 2) {
                        blocks = (int) Math.min(deltaY, maxPower.get() ? 170 : fallHeight.get());
                    } else if (deltaY < -1) {
                        blocks = (int) Math.min(-deltaY, maxPower.get() ? 170 : fallHeight.get());
                    } else {
                    ...
                    if (isSafeBlock(isopenair1) && isSafeBlock(isopenair2)) {
                        if (blocks <= 22) {
                            if (mc.player.hasVehicle()) {
                                for (int launch = 0; launch < launchCount.get(); launch++) {
